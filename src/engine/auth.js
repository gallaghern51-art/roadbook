...
function friendly(err) {
  ...
  if (/manual linking|identity linking/i.test(msg)) return 'Account upgrades are not enabled yet. Try again in a moment.';
  if (/auth session missing/i.test(msg)) return 'That account link has expired. Start again from Create account.';
...
export async function signUp({ email, password, name }) {
  ...
  try {
    ...
    if (existing?.is_anonymous) {
      ...
      }, { emailRedirectTo: redirectTo('finish') });
      if (error) throw error;
      // Supabase requires the email identity to be verified before an
      // anonymous user can add a password. Projects without email
      // confirmation may complete that first step immediately; otherwise the
      // confirmation link returns to the password-only finish screen.
      if (isAccount(data.user)) {
        const { data: passwordData, error: passwordError } = await supabase.auth.updateUser({ password });
        if (passwordError) throw passwordError;
        return { user: passwordData.user, needsConfirmation: false };
      }
...
function redirectTo(flow) {
  try {
    const url = new URL(window.location.origin + window.location.pathname);
    if (flow) url.searchParams.set('account', flow);
    return url.toString();
  } catch { return undefined; }
}

function requestedFlow() {
  try { return new URLSearchParams(window.location.search).get('account'); } catch { return null; }
}

function clearRequestedFlow() {
  try {
...
export function useAuth() {
  ...
  const [recovery, setRecovery] = useState(() => requestedFlow() === 'recovery');
  const [finishAccount, setFinishAccount] = useState(() => requestedFlow() === 'finish');
  ...
  useEffect(() => {
    ...
    currentUser()
      .then((u) => {
        if (!alive) return;
        setUser(u);
        // A stale/expired confirmation URL must fall back to ordinary sign-in,
        // not strand the visitor on a password form with no session.
        if (!isAccount(u)) setFinishAccount(false);
        setStatus('ready');
      })
    ...
    const { data } = supabase.auth.onAuthStateChange((event, session) => {
      ...
      if (requestedFlow() === 'finish' && isAccount(session?.user)) setFinishAccount(true);
  ...
  return {
    ...
    status,
    recovery,
    clearRecovery: () => { setRecovery(false); clearRequestedFlow(); },
    finishAccount,
    clearFinishAccount: () => { setFinishAccount(false); clearRequestedFlow(); },
