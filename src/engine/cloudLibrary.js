...
function fingerprint(rec) {
  return hash(JSON.stringify([rec.name, rec.trip, rec.scenarios ?? [], rec.chat ?? [], rec.remote ?? null]));
}
...
export function useLibraryBackup(state, dispatch, account) {
  ...
  }, [userId, state.lib, state.trip, state.scenarios, state.chat, state.remote]);
