...
export default function RouteSheet({ ride, onChange, onGo, onClose, onOptions, onChooseOnMap, pace = 1, busyLabel }) {
  const t = useT();
  const u = useUnits();
  ...
  useEffect(() => {
    ...
    setBusy(true); setErr('');
    routeOptions({ start, stops, end, avoidTolls, pace, signal: ctl.signal })
      .then((list) => {
