...
## The home map (option A) — built (same day)
The owner chose option A from the canvas mockup (Roadbook Home Map View: today's home beside Map first / Two decks / Map as a tab, then A built out as five screens). `HomeMap.jsx` + the restructured `Home.jsx`: the Mapbox map is the front door, the pill is both doors (place name → places; sentence → the AI builder), the sheet peeks with the two verbs and the trips row and pulls up to the old home, and a tapped place is a card with Ride here (quick ride) / Add to a trip (ranked by the nearest stop, day pre-picked) / Details. `home-map-check` 54/54 at 375 and 1280; the CLAUDE.md note carries the detail.
