...
| `live-settings.mjs` | **36/36 (live)** | The settings buildout end to end: nine sections, a home address searched through a real geocoder and stored with real coordinates, the bike catalog deriving the range the feasibility engine grades against (and being overridable), stated dietary/avoid requirements, survival across a reload, and the planner payload actually carrying home + taste + range. Restores the account profile afterwards. |
| `pwa-shell-check.mjs` | **33/33** | iOS Home Screen + Safari shell: emulates Apple’s `navigator.standalone` path without rewriting CSS, verifies the shell fills and cannot scroll, that the mode bar reaches the bottom of the box with no dead strip below it, that the top chrome tracks `env(safe-area-inset-top)` exactly and adds no phantom inset of its own (the opaque-status-bar contract), that safe areas survive panel transitions, that ordinary Safari keeps its dynamic viewport, and that an idle map has zero marker churn or redraw feedback. Covers the app dragging up, the bottom void, map furniture above the masthead, panel-close jump under the Dynamic Island, and flashing. |
| `pin-drop-check.mjs` | **81/81** | A long press (touch) / right-click (mouse) on open map drops a NEEDLE pin with the wheel (tip on the pressed pixel, head 40 px above, ≥44pt ✓/✕/ⓘ, five-place readout, drag moves the tip with the pointer and re-names once on settle, no card until ✓) on both maps: the pure gesture helper over a fake map (tap never drops, hold fires once, pan/pinch/map-move cancel, a marker's press is the marker's, the release of a held finger fires when the engine ran no timer, the hold measured on the events' own timeStamps against WebKit's deferred dispatch — the iOS path, contextmenu dedupe, `recent()` swallow), the reverse-geocode label parsers, then the browser at 375 (touch + mobile) and 1280: the home card as a placed spot named by its road after one reverse-geocode call, Ride here → a quick ride to the placed coordinate, Add to a trip → `placed: 'rider'` in route order with the day panel's ◎ tag, the trip map's PoiCard as the same placed spot with Add to this day, tap-to-add still the naming sheet (now stamped placed), no `window.prompt`. Takes `RB_PORT` when 5199 is busy. |
| `stop-sheet-check.mjs` | **26/26** | Tapping a stop marker opens the stop's own PlaceSheet: the day's schedule at that stop (arrive · on the ground · roll out · the leg in), the honest tag (✓ / ◎ placed / ⚠ unverified), the rider's note, one details + one photo call for a verified stop and none for a placed one, Edit stop → the editor, Escape closes; drag-to-move intact and a moved marker becomes `placed: 'rider'`; a marker on the line keeps its own hover tooltip; a finger held on a marker drops no pin under it. `RB_PORT` as above. |
| `landing-check.mjs` | **66/66** | The signed-out front door, Direction C: one ride told in six chapters (intake, narration, the app's own builder desk, the grade, the Prep board, Ride Mode, a shared template), dark whatever theme is stored, no toggle, Ride Mode as the frame the app would draw (OpenStreetMap's Red Lodge + the router's real US-212 as a tilted SVG plane, zero Mapbox requests — asserted), the puck, and the HUD reading the router's real distance, three steps / six features / two real-data templates, primary CTA above the fold, Sign in → card focus, guest door → Home, 44px/16px on a phone. Needs Supabase keys in `.env.local` (no gate otherwise); `RB_PORT` overrides 5199. |
