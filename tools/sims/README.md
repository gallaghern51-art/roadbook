...
| `pwa-shell-check.mjs` | **11/11** | Installed-PWA shell: with `display-mode: standalone` forced through the real stylesheet, an over- or under-estimated `--app-h` must never let the DOCUMENT scroll. Covers all three field reports — the app dragging up, the map furniture ending up above the masthead, and the view jumping under the Dynamic Island on a panel close. On the pre-fix CSS it scores 4/11 with the masthead at top −40. |
