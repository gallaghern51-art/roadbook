...
| `pin-drop-check.mjs` | **59/59** | A long press (touch) / right-click (mouse) on open map drops a PLACED pin on both maps: the pure gesture helper over a fake map (tap never drops, hold fires once, pan/pinch/map-move cancel, a marker's press is the marker's, the release of a held finger fires when the engine ran no timer, the hold measured on the events' own timeStamps against WebKit's deferred dispatch — the iOS path, contextmenu dedupe, `recent()` swallow), the reverse-geocode label parsers, then the browser at 375 (touch + mobile) and 1280: the home card as a placed spot named by its road after one reverse-geocode call, Ride here → a quick ride to the placed coordinate, Add to a trip → `placed: 'rider'` in route order with the day panel's ◎ tag, the trip map's PoiCard as the same placed spot with Add to this day, tap-to-add still the naming sheet (now stamped placed), no `window.prompt`. Takes `RB_PORT` when 5199 is busy. |
