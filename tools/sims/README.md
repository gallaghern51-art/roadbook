...
| `landing-check.mjs` | **58/58** | The signed-out front door, Direction C: one ride told in six chapters (intake, narration, the app's own builder desk, the grade, the Prep board, Ride Mode, a shared template), dark whatever theme is stored, no toggle, Ride Mode as the frame the app would draw (OpenStreetMap's Red Lodge + the router's real US-212 as a tilted SVG plane, zero Mapbox requests — asserted), the puck, and the HUD reading the router's real distance, three steps / six features / two real-data templates, primary CTA above the fold, Sign in → card focus, guest door → Home, 44px/16px on a phone. Needs Supabase keys in `.env.local` (no gate otherwise); `RB_PORT` overrides 5199. |
