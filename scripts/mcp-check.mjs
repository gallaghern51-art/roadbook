...
console.log('auth:');
{
  ...
  check('the header description is folded to ASCII so a prose message can never turn a 401 into a 500', /^[\x20-\x7e]+$/.test(hdr2) && /not valid - make a new one in Roadbook > Settings > Connect your AI/.test(hdr2) && (() => { try { new Response('', { status: 401, headers: { 'WWW-Authenticate': hdr2 } }); return true; } catch { return false; } })(), hdr2);
}
