...
- Phases (storage keys, not words the rider sees): "prep" for arrival/pickup/staging days, "outbound" for the way out, "rally" for the days spent AT the destination — an event, a base town, a loop area, whatever the trip is for — and "return" for the way home. The rider names these phases per trip; never assume a rally.
